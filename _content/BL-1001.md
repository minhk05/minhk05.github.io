...
>>(1,2)
```
주어진 데이터에서 원하는 부분을 슬라이싱을 통해 사용할 수 있어야 한다.   
주어진 데이터의 Shape을 변형시킬 수 있어야 한다.
타 프레임워크에 필요한 쉐잎이 Matrix인데, 슬라이싱을 Vector로 해버리면 Shape이 맞지 않는다는 에러가 남.
